...
public class EasyReader implements IReader<EasySocketOptions> {
    ...
    private OriginReadData originReadData;
    @Override
    public void read() throws IOException, ReadRecoverableExeption, ReadUnrecoverableException {
        ...
        // 保存header
        originReadData.setHeaderData(headBuf.array());

        /*2、读 body=====>>>*/
        int bodyLength = messageProtocol.getBodyLength(originReadData.getHeaderData(), socketOptions.getReadOrder());
        if (bodyLength > 0) {
            ...
            if (remainingBuf != null) {
                ...
                if (length == bodyLength) {
                    ...
                    // 保存body
                    originReadData.setBodyData(bodyBuf.array());

              //      LogUtil.d("Socket收到数据-->" + originalData.getBodyString());
                    // 分发数据
                    actionDispatch.dispatchAction(IOAction.ACTION_READ_COMPLETE, originReadData);
       ...
       LogUtil.d("Socket收到数据1-->" + originReadData.getBodyString());
        // 分发
        actionDispatch.dispatchAction(IOAction.ACTION_READ_COMPLETE, originReadData);
    ...
    private Runnable readerTask = new Runnable() {
        ...
        public void run() {
            try {
                ...
                Log.d("cppa ",readRecoverableExeption.toString());
...
//            throw new ReadRecoverableExeption("读数据失败，可能因为socket跟服务器断开了连接");
//        }*/
//        LogUtil.d("Socket收到数据2-->" + Arrays.toString(data));
...
//        originBuf.clear();
        ...
        if (this.originReadData == null) {
            this.originReadData = readData;
        }

        int len = this.inputStream.read(this.originBuf.array());
        Log.d("soc", "len " + len);
        if (len == -1) {
            throw new ReadRecoverableExeption("读数据失败，可能因为socket跟服务器断开了连接");
        } else {
            byte[] data = new byte[len];
            this.originBuf.get(data, 0, len);
            LogUtil.d(new String[]{"Socket收到数据-->" + new String(data)});
            if ((new String(data)).endsWith("\n")) {
                this.originReadData.addBodyData(data);
                readData.addBodyData(data);
                this.originBuf.clear();
                OriginReadData sendData = new OriginReadData();
                sendData.setHeaderData(this.originReadData.getHeaderData());
                sendData.setBodyData(this.originReadData.getBodyBytes());
                LogUtil.d(new String[]{"Socket发送数据-->" + new String(sendData.getOriginDataBytes())});
                this.actionDispatch.dispatchAction("action_read_complete", sendData);
                this.originReadData.clearBodyData();
            } else {
                this.originReadData.addBodyData(data);
                readData.addBodyData(data);
                this.originBuf.clear();
